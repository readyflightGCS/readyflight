...
export default function GeofenceLayer() {
  ...
  return (
    <LayerGroup>
      {selectedSubMission == 'Geofence'
        ? filterLatLngCmds(mission.flatten('Geofence'), dialect).map((waypoint, idx) => {
            let active = false
            const x = mission.findNthPosition('Geofence', idx)
            ...
            return (
              <DraggableMarker
                key={idx}
                position={getCommandLocation(waypoint, dialect)}
                onMove={(lat, lng) => onMove(lat, lng, idx)}
                active={active}
              />
            )
          })
      ...
      {mission.flatten('Geofence').length > 0 ? (
        <Polygon
          ...
          positions={[
            ...
            filterLatLngCmds(mission.flatten('Geofence'), dialect).map((x) =>
              getCommandLocation(x, dialect)
            )
          ]}
        />
      ) : null}

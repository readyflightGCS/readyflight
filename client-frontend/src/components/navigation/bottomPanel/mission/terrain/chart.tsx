...
const CustomTooltipContent = ({
  ...
  if (active && payload && payload.length) {
    const commandEntry = payload.find(
      (pld: { dataKey: string; value?: string | null }) =>
        pld.dataKey === 'commandHeight' && pld.value !== undefined && pld.value !== null
    )
...
export default function TerrainChart({
  ...
  if (commandPositions.length < 1 && terrainProfile.length === 0) {
    // Changed to < 1 for commandPositions
    return (
  ...
  commandPositions.forEach((pos, index) => {
    // Add index for accessing gradient
    if (commandPointsWithDistance.length > 0) {
      // Check based on array instead of index
      const prevPos = commandPositions[commandPointsWithDistance.length - 1] // Get actual previous position
      cumulativeCmdDistance += haversineDistance(
    ...
    <ResponsiveContainer width="100%" height={185}>
      <ComposedChart data={chartData} margin={{ top: 30, right: 20, bottom: 20, left: 10 }}>
        ...
        <Scatter
          ...
          shape={
            // @ts-ignore - payload is missing, included by Scatter i think
            <CommandDot onCmdClick={onCommandClick} />
          } // Pass onCommandClick to the shape
        />
      </ComposedChart>
    </ResponsiveContainer>
  )

...
export default function HeightMap() {
  ...
  if (wps.length < 2) {
    return (
      <div className="h-[150px] flex w-full items-center justify-center">
        Place two or more waypoints for height map
      </div>
    )
  }
  ...
  function onChange(event: { target: { name: string; value: number; delta: number } }) {
    ...
    newWps.changeManyParams(
      ...
      selectedSubMission,
      (cmd) => {
        cmd.params['altitude'] += event.target.delta
        return cmd
      },
      true
    )
  ...
  function autoHeight() {
    ...
    for (let i = 0; i < wps.length && selectedCommandIndex < selectedCommands.length; i++) {
      ...
      if (
        wp === selectedCmd &&
        'altitude' in wp.params &&
        'latitude' in wp.params &&
        'longitude' in wp.params
      ) {
        flattenedIDs.push(i)
    ...
    const validWaypoints = wps.filter(
      (wp) => 'altitude' in wp.params && 'latitude' in wp.params && 'longitude' in wp.params
    ) as Array<{ params: { latitude: number; longitude: number; altitude: number } }>

    if (validWaypoints.length < 2) return

    const { interpolatedAltitudes } = calculateInterpolatedAltitudes(
      validWaypoints,
      0,
    ...
    <div className="w-full p-2">
      <div className="flex flex-row gap-2">
        ...
        <label>
          ...
          <select
            value={frameAllSame ? '' + frameVal : ''}
            onChange={changeFrame}
            className="w-40 h-[25px] border-input bg-card"
          >
            {!frameAllSame ? (
        ...
        {selected.length > 2 ? (
          <Button variant="active" onClick={autoHeight}>
            Auto Height
          </Button>
        ) : null}
      </div>

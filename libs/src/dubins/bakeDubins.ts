...
export function createEvaluate(
  ...
  function evaluate(x: number[]): number {
    ...
    const path = dubinsBetweenDubins(localWPS)
...
export function staticEvaluate(
  ...
  for (const section of dubinSections) {
    const dubinsPoints: dubinsPoint[] = section.run.map((x) =>
      waypointToDubins(x.cmd, reference, dialect)
    )

    const startingParams = [...getTunableDubinsParameters(dubinsPoints)]
    const bounds: bound[] = [...getBounds(dubinsPoints, vehicle)]
    applyBounds(startingParams, bounds)

    const evaluate = createEvaluate(dubinsPoints, optimisationFunction)
...
export function bakeDubins(
  ...
  const curWaypoints = waypoints.clone()
  ...
  for (const section of dubinSections) {
    ...
    const result = optimisationmethod(startingParams, bounds, evaluate) // 2041

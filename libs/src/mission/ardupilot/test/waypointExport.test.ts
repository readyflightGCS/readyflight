...
test('.waypoints export uses QGC WPL 110 and includes reference waypoint + mission waypoint', async () => {
  ...
  mission.pushToMission(
    'Main',
    makeCommand('RF.Waypoint', { latitude: 10.11, longitude: 20.02, altitude: 123 }, ardupilot)
  )
...
test('.waypoints exports various types correctly', async () => {
  ...
  mission.pushToMission(
    'Main',
    makeCommand(
      'D.MAV_CMD_NAV_TAKEOFF',
      { latitude: 10.11, longitude: 21.02, altitude: 123 },
      ardupilot
    )
  )
  mission.pushToMission(
    ...
    makeCommand(
      ...
      { latitude: 10.11, longitude: 20.02, altitude: 123 },
      ardupilot
    )
  )
  mission.pushToMission(
    ...
    makeCommand(
      'D.MAV_CMD_NAV_WAYPOINT',
      { latitude: 11.11, longitude: 20.02, altitude: 123 },
      ardupilot
    )
  )
  mission.pushToMission(
    ...
    makeCommand(
      'D.MAV_CMD_NAV_WAYPOINT',
      { latitude: 11.11, longitude: 21.02, altitude: 123 },
      ardupilot
    )
  )
  mission.pushToMission(
  ...
  mission.pushToMission(
    'Main',
    makeCommand('D.MAV_CMD_NAV_LAND', { latitude: 10.11, longitude: 20.02, altitude: 0 }, ardupilot)
  )

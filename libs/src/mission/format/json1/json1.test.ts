...
test('readyflight json1 export matches schema', async () => {
  ...
  mission.pushToMission(
    'Main',
    makeCommand('RF.Waypoint', { latitude: 51.501, longitude: -0.121, altitude: 42 }, ardupilot)
  )
...
test('readyflight json1 import accepts exported blob and returns mission + vehicle', async () => {
  ...
  mission.pushToMission(
    'Main',
    makeCommand('RF.Waypoint', { latitude: 1.001, longitude: 2.002, altitude: 50 }, ardupilot)
  )
